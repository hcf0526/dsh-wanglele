...
export function getVisibleCustomPromptState(agent, pluginName = name) {
  ...
  // DSH current Session exposes its durable log as `log`/`snapshotEvents()`;
  // older hosts exposed the same data as `events`. Read the current API first.
...
export function resolveSettings(raw) {
  ...
  return {
    ...
    autoRetryEnabled: typeof raw.autoRetryEnabled === "boolean"
      ? raw.autoRetryEnabled
      : DEFAULT_SETTINGS.autoRetryEnabled,
    autoRetryDelaySeconds: clampInteger(
      raw.autoRetryDelaySeconds,
...
export function turnHasAssistantText(events, turn) {
  ...
  for (const event of events) {
    ...
    if (event.type === "assistant/message") {
      if (contentHasVisibleText(event.data?.message?.content)) return true;
      continue;
    }
...
export function evaluateAutoRetry({
  ...
  const current =
    typeof consecutiveCompleteFailures === "number" && consecutiveCompleteFailures > 0
      ? consecutiveCompleteFailures
      : 0;
  const limit =
    typeof maxFailures === "number" && maxFailures >= AUTO_RETRY_MIN_FAILURES
      ? maxFailures
      : DEFAULT_SETTINGS.autoRetryMaxFailures;

  if (!enabled) {
    return { action: "idle", consecutiveCompleteFailures: 0 };
  }
  if (reasonKind === "aborted") {
    return { action: "cancel", consecutiveCompleteFailures: 0 };
  }
  if (reasonKind !== "error") {
    return { action: "reset", consecutiveCompleteFailures: 0 };
  }

  const nextCount = hasAssistantText ? 0 : current + 1;
  if (!hasAssistantText && nextCount >= limit) {
    return { action: "stop", consecutiveCompleteFailures: nextCount };
  }
...
export function createAutoRetryEngine({
  getSettings,
  ...
  async function onSessionEvent(session, event, ctx) {
    if (!isRecord(event) || !isRecord(session)) return;
    const settings = typeof getSettings === "function" ? resolveSettings(getSettings()) : resolveSettings({});
    ...
    if (event.type === "user/message") {
      ...
      if (kind === "user" && typeof id === "string" && !state.issuedIds.has(id)) {
        cancel(session);
        state.consecutiveCompleteFailures = 0;
      }
      return;
    ...
    const decision = evaluateAutoRetry({
      ...
      hasAssistantText: turnHasAssistantText(
    ...
    const live = typeof getSettings === "function" ? resolveSettings(getSettings()) : settings;
    if (!live.autoRetryEnabled) return;
    ...
    try {
      ...
      ctx?.logger?.warn?.(`[wanglele] auto-retry followup failed: ${err?.message || err}`);
    }
  }
...
export function apply(ctx, config = {}) {
    ...
    ctx.inject(["settings"], (sctx) => {
      try {
          ...
          (input) => resolveSettings(input),
          {
            toJSON: () => ({
              ...
              properties: {
                ...
                autoRetryEnabled: { type: "boolean", default: DEFAULT_SETTINGS.autoRetryEnabled },
                autoRetryDelaySeconds: { type: "number", default: DEFAULT_SETTINGS.autoRetryDelaySeconds },
                autoRetryMaxFailures: { type: "number", default: DEFAULT_SETTINGS.autoRetryMaxFailures },
              },
            }),
          },
        );
        ...
        scope.watch((next) => {
          ...
          if (!liveSettings.autoRetryEnabled) autoRetry.dispose();
        });
      } catch (err) {
  ...
  ctx.on(
    "session/event",
    (session, event) => {
      void autoRetry.onSessionEvent(session, event, ctx);
    },
    { global: true },
  );
}
