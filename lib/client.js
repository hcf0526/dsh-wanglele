window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      autoRetrySection: "自动重试",
      enableAutoRetry: "启用自动重试",
      autoRetryDelay: "失败后延时",
      autoRetryDelayHint: "对话失败后等待多少秒再发送「继续」，默认 5 秒",
      autoRetryMaxFailures: "最大连续完全失败次数",
      autoRetryMaxFailuresHint: "模型连续多次未输出任何文字后停止自动重试",
      autoRetrySeconds: "秒",
      autoRetryComposer: "自动重试",
      autoRetryOn: "开",
      autoRetryOff: "关",
      savedSuccess: "已保存",
    };
    ...
    const en = {
      ...
      autoRetrySection: "Auto Retry",
      enableAutoRetry: "Enable auto retry",
      autoRetryDelay: "Retry delay",
      autoRetryDelayHint: "Seconds to wait after a failed turn before sending Continue. Default 5.",
      autoRetryMaxFailures: "Max consecutive empty failures",
      autoRetryMaxFailuresHint: "Stop auto retry after this many consecutive turns with no model text.",
      autoRetrySeconds: "sec",
      autoRetryComposer: "Auto retry",
      autoRetryOn: "On",
      autoRetryOff: "Off",
      savedSuccess: "Saved",
    };
    ...
    const css = `
      ...
      .wll-chip .wll-slider:before {
        height: 14px;
        width: 14px;
        left: 2px;
        bottom: 2px;
      }
      .wll-chip .wll-switch input:checked + .wll-slider:before {
    ...
    function WangleleSettingsSection({ t, scope }) {
      ...
      const autoRetryEnabled = value.autoRetryEnabled ?? false;
      const autoRetryDelaySeconds = typeof value.autoRetryDelaySeconds === "number"
        ? value.autoRetryDelaySeconds
        : 5;
      const autoRetryMaxFailures = typeof value.autoRetryMaxFailures === "number"
        ? value.autoRetryMaxFailures
        : 3;
      ...
      return h(
        ...
        h(
          ...
          h(
            "div",
            { className: "wll-card-header" },
            h("div", { className: "wll-card-title" }, t("autoRetrySection")),
          ),
          h(
            "div",
            { className: "wll-row" },
            h(
              "div",
              { className: "wll-row-info" },
              h("div", { className: "wll-row-label" }, t("enableAutoRetry")),
            ),
            h(
              "label",
              { className: "wll-switch" },
              h("input", {
                ...
                onChange: (e) => updateSetting("autoRetryEnabled", e.target.checked),
              }),
              h("span", { className: "wll-slider" }),
            ),
          ),
          ...
          h(
            ...
            h(
              "div",
              { className: "wll-row-info" },
              h("div", { className: "wll-row-label" }, t("autoRetryDelay")),
              h("div", { className: "wll-row-hint" }, t("autoRetryDelayHint")),
            ),
            h(
              "div",
              { className: "wll-number-field" },
              h("input", {
                ...
                onChange: (e) => {
                  ...
                  if (Number.isFinite(next)) updateSetting("autoRetryDelaySeconds", next);
                },
              }),
              h("span", { className: "wll-row-hint" }, t("autoRetrySeconds")),
            ),
          ),
          h("div", { className: "wll-divider" }),
          h(
            ...
            h(
              "div",
              { className: "wll-row-info" },
              h("div", { className: "wll-row-label" }, t("autoRetryMaxFailures")),
              h("div", { className: "wll-row-hint" }, t("autoRetryMaxFailuresHint")),
            ),
            h("input", {
              className: "wll-number",
              type: "number",
              ...
              onChange: (e) => {
                const next = Number(e.target.value);
                if (Number.isFinite(next)) updateSetting("autoRetryMaxFailures", next);
              },
            }),
          ),
        ),
    ...
    function AutoRetryComposerToggle({ t, scope }) {
      const snapshot = React.useSyncExternalStore(
        (cb) => (scope?.subscribe ? scope.subscribe(cb) : () => {}),
        () => (scope?.getSnapshot ? scope.getSnapshot() : { value: {} }),
        () => ({ value: {} }),
      );
      const enabled = snapshot?.value?.autoRetryEnabled ?? false;

      return h(
        "label",
        {
          className: `wll-chip${enabled ? " wll-chip-on" : ""}`,
          title: t("enableAutoRetry"),
        },
        `${t("autoRetryComposer")} ${enabled ? t("autoRetryOn") : t("autoRetryOff")}`,
        h(
          "span",
          { className: "wll-switch" },
          h("input", {
            ...
            checked: enabled,
            onChange: (e) => {
              if (scope?.set) scope.set("autoRetryEnabled", e.target.checked);
            },
          }),
          h("span", { className: "wll-slider" }),
        ),
      ...
      ctx.slots.inject("conversation.input.right", () => ctx.slots.register(
        {
          ...
          order: 10,
          label: () => t("autoRetryComposer"),
          locale: NS,
          inject: () => ({ t, scope: boundScope }),
        },
        AutoRetryComposerToggle,
      ));
    }
