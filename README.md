...
### 3. 🔁 对话失败自动重试
- 输入栏提供「自动重试」滑动开关，设置页可配置延时秒数（默认 5 秒）与最大连续完全失败次数。
- 仅在对话运行失败时延时发送「继续」。成功结束的对话不会触发。
- 模型连续多次未输出任何文字记为完全失败并停止自动重试；某次「继续」后只要模型回复了文字，连续计数清零。

### 4. 🔄 Gemini 3 Flash 流式工具调用自动恢复
